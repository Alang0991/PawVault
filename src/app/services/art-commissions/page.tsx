...
export default function ArtCommissionsPage() {
  const [commissioners, setCommissioners] = useState<ArtCommissioner[]>([])
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const res = await fetch(`/api/commissioners/art?${params.toString()}`)
    ...
    load()
  }, [activeTab, categoryFilter, availabilityFilter, typeFilter, styleFilter, searchQuery, sortBy, priceRange])
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h1 className="text-4xl font-bold text-text-primary mb-4">Art Commissioners Directory</h1>
              <p className="text-lg text-text-secondary max-w-2xl mx-auto">
                Discover talented 2D artists for character art, illustrations, reference sheets, and more.
              </p>
            </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <aside className="lg:col-span-1 space-y-6">
              ...
              <Card>
                <CardContent className="p-4">
                  <h3 className="font-semibold text-text-primary mb-4">Art Styles</h3>
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {ART_CATEGORIES.map((style) => (
                      <label key={style} className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={styleFilter === style}
                          onChange={(e) => setStyleFilter(e.target.checked ? style : null)}
                          className="rounded border-input text-accent focus:ring-accent"
                        />
                  ...
                  {commissioners.length === 0 ? (
                    <Card>
                      <CardContent className="p-12 text-center">
                        ...
                        <p className="text-text-secondary">No artists match your filters.</p>
                      </CardContent>
                    </Card>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {commissioners.map((commissioner) => (
                        <ArtCommissionerCard key={commissioner.id} commissioner={commissioner} />
                      ))}
                    </div>
                  )}
                </TabsContent>
                ...
                {["featured", "verified", "open"].map((tab) => (
                  <TabsContent key={tab} value={tab} className="space-y-4">
                    {commissioners.length === 0 ? (
                      <Card>
                        <CardContent className="p-12 text-center">
                          <Palette className="h-12 w-12 mx-auto mb-4 text-text-muted" />
                          <p className="text-text-secondary">No {tab} artists at the moment.</p>
                        </CardContent>
                      </Card>
                    ) : (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
...
function ArtCommissionerCard({ commissioner }: { commissioner: ArtCommissioner }) {
      ...
      <CardContent className="p-5">
        <div className="flex items-start gap-4">
          <Avatar className="h-14 w-14 shrink-0 ring-2 ring-background -mt-7">
            ...
            <AvatarFallback className="text-lg bg-gradient-to-br from-pink-500 to-rose-500 text-white">
          ...
          <div className="flex-1 min-w-0 mt-1">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {commissioner.artStyles.slice(0, 4).map((style) => (
                <Badge key={style} variant="outline" className="text-xs">
                  {style}
                </Badge>
              ))}
              {commissioner.artStyles.length > 4 && (
        ...
        <div className="mt-4 flex items-center justify-between pt-4 border-t">
          ...
          <Link
            href={`/commissioners/art/${commissioner.id}`}
