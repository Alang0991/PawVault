...
export async function GET(request: Request) {
  try {
    ...
    const commissioners = await prisma.artCommissioner.findMany({
      where,
      orderBy,
      take: limit,
      include: {
    ...
    console.error("Failed to fetch art commissioners:", error)
    return NextResponse.json({ error: "Failed to fetch art commissioners" }, { status: 500 })
  }
}